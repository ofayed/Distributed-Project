                        ...
                        for i in 0..imagnum
                        {
                            ...
                            while let Ok(bytes) = socket.recv(&mut buffer).await {
                                ...
                                if received_data.len() as u64 == expected_image_size {
                                            ...
                                            let path = String::from_utf8_lossy(data2).to_string(); 
    ...
    loop {
        ...
        while let Ok(bytes) = socket.recv(&mut buffer).await {
            ...
            if received_data.len() as u64 == expected_image_size {
                ...
                let mut combined_string = "encryption/".to_string() + newpath.as_str() +  ".jpg" ;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let socket = UdpSocket::bind("127.0.0.1:1038").await?;
    let flag2 = 0;
    let mut images: Vec<(String, i32)> = Vec::new();
    ...
    match send_message_with_retransmission(
        ...
        else if c==4
        {
            ...
            let mut input = String::new();
            io::stdin().read_line(&mut input).expect("Failed to read line");
            let newac: i32 = input.trim().parse().expect("Please type a number!");

            update_access("update", "127.0.0.1:1008".parse::<SocketAddr>().unwrap(), &socket,newac, &images_sent[index]);
            update_access("update", "127.0.0.1:1018".parse::<SocketAddr>().unwrap(), &socket,newac, &images_sent[index]);
            update_access("update", "127.0.0.1:1028".parse::<SocketAddr>().unwrap(), &socket,newac, &images_sent[index]);
            }
